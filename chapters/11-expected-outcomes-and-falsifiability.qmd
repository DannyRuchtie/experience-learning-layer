...
::: {.callout-important}
**Superseded.** The sizing in this section described the v0.6 contract, which is no longer canonical.
The generator does not produce 640 paired sealed tasks, the discordance rate was an unjustified
default, and v0.7 makes the latent rule rather than the task the unit of analysis — while itself
declaring 36 sealed rules against its own cluster-power requirement of 117 to 208. **No sizing is
currently preregistered.** Tier counts, rule counts and the discordance rate are empirical outputs of
the repaired instrument and will be set in v0.8.
:::

The v0.6 contract used 640 paired sealed tasks per confirmatory condition, on the claim that this
exceeds the approximate 625 pairs required for 80% power under a two-sided 0.05 test, a five-point
effect, and an assumed 0.20 discordant-pair rate. The primary interval is a 10,000-resample paired percentile bootstrap with a committed analysis seed. Predeclared corrupt source artifacts and duplicate task identifiers are the only eligible exclusions; provider failures remain scored failures with their traces retained.

## Outcome labels

![**The verdict reports both benefit and safety.** Supported requires every gate; partially supported names the failed gate; not supported redirects work to the stronger or simpler method; unsafe disqualifies deployment.](assets/visuals/outcome-labels.png){#fig-outcome-labels fig-alt="Four verdict cards define supported, partially supported, not supported, and unsafe outcomes." width="100%"}
